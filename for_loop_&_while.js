...
for(let i=1; i<=50; i++) {
  let stringValue = i.toString() //toString함수를 사용하는건데 끝에는 () 가 들어간다.
  let result = "" // 우리가 나중에 프린트 하고 싶은 결과 값을 적은 것이다
  // 3,6,9 에 "짝" 을 인식하도록 설정해야 한다.
  for(let j=0;j<stringValue.length;j++) {
    if(stringValue[j]==="3" || stringValue==="6" || stringValue[j]==="9"){
      result = result+"짝" // = result += "짝"
    } 
    // j는 인덱스 값에서 어떤 숫자를 대입할 때 3이나6이나9가 들어가면 result에 "짝"이 들어가도록 설정한다.
    // result = "짝" 이라고 하면 그냥 33 이런것도 "짝" 하나로 나오기 때문에
    // result = result + "짝" 으로 설정해야 33 이런숫자에 "짝짝" 이렇게 나오게 된다.
    // 이 의미가 기존에 있는 result 값에다가 "짝"을 추가해 주는 것이다.
  }
  //console.log(result) // 우리가 뭘 프린트 할지는 몰라도 우선 그걸 result로 남겨둔다.
  //현재 다른 console.log없이 result만 console.log찍으니까 "짝"만 보이는 것이다.
  
   console.log(result.length>0?result:i)
  // 만약의 reuslt 길이가 0보다 크다. 즉,글자가 하나이상 들어 있으면은? result를 프린트 해줘야 한다. 
  // 그러나(:) (result에 아무것도 없으면은) i를 프린트 해라.
}

// 39 에서 두번 짝짝 해야되는 부분에 대해서
// 컴퓨적 인식으로는 39 자체가 아니라 3따로 9따로 인식하게 해서 짝짝 하게 하는 것이다.
// 
